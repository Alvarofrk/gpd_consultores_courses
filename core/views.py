...
def cotizacion_download_pdf(request, pk):
    ...
    p.setFont("Helvetica", 9)
    p.drawString(width-80, y_tot, f"S/ {math.ceil(float(cotizacion.total_con_detraccion) * 10) / 10:.1f}")
    ...
    p.setFont("Helvetica", 8)
    p.drawString(width-80, y_tot-30, f"S/ {math.ceil(float(cotizacion.total_con_igv) * 10) / 10:.1f}")
    p.setFont("Helvetica-Bold", 8)
    p.drawString(width-300, y_tot-45, "12% DE DETRACCIÓN - Banco de la Nación*")
    p.setFont("Helvetica", 8)
    p.drawString(width-80, y_tot-45, f"S/ {math.ceil(float(cotizacion.detraccion) * 10) / 10:.1f}")
    p.setFont("Helvetica-Bold", 8)
    p.drawString(width-300, y_tot-60, "INVERSIÓN TOTAL A DEPOSITAR")
    p.setFont("Helvetica", 9)
    p.drawString(width-80, y_tot-60, f"S/ {math.ceil(float(cotizacion.total_con_detraccion) * 10) / 10:.1f}")
